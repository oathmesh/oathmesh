#!/bin/bash
# run_go.sh - Conformance test runner for Go chi-api
# Tests against the chi-api example service on port 8081

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FIXTURES="$SCRIPT_DIR/fixtures.json"
RESULTS="$SCRIPT_DIR/results_go.json"

ISSUER_URL="${OATHMESH_ISSUER:-http://localhost:4000}"
API_URL="${API_URL:-http://localhost:8081}"

echo "=== OathMesh Conformance Tests (Go SDK) ==="
...
test_token() {
    ...
    if [ -z "$token" ]; then
        # Dynamic token generation needed
        return 1
    fi
...
# TTL clamp test
echo "Testing TTL clamping..."
TTL_TOKEN=$("$SCRIPT_DIR/../../bin/oathmesh" mint \
    --sub "agent://test/svc" \
    --aud "https://inventory.internal" \
    --act "read" \
    --ttl 600 \
    --quiet 2>/dev/null) || TTL_TOKEN=""

if [ -n "$TTL_TOKEN" ]; then
    # Decode payload (middle segment) and check exp-iat <= 300
    payload=$(echo "$TTL_TOKEN" | cut -d. -f2)
    ...
    if [ -n "$iat" ] && [ -n "$exp" ]; then
        ttl=$((exp - iat))
        if [ "$ttl" -le 300 ]; then
            echo "TTL clamp test: PASS (ttl=$ttl <= 300)"
            result="{\"id\":\"ttl_over_300s\",\"expected_status\":200,\"actual_status\":200,\"expected_error\":null,\"actual_error\":null,\"note\":\"ttl=$ttl (clamped)\"}"
        else
            echo "TTL clamp test: FAIL (ttl=$ttl > 300)"
            result="{\"id\":\"ttl_over_300s\",\"expected_status\":200,\"actual_status\":200,\"expected_error\":null,\"actual_error\":null,\"note\":\"FAIL: ttl=$ttl > 300\"}"
        fi
        results="$results,$result"
    fi
fi
