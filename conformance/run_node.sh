#!/bin/bash
# run_node.sh - Conformance test runner for Node.js Express API
# Tests against the Express example service on port 3000

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FIXTURES="$SCRIPT_DIR/fixtures.json"
RESULTS="$SCRIPT_DIR/results_node.json"

ISSUER_URL="${OATHMESH_ISSUER:-http://localhost:4000}"
API_URL="${API_URL:-http://localhost:3000}"

echo "=== OathMesh Conformance Tests (Node SDK) ==="
